...
x=0
#command to get the number of cors
...
#get the name of the cors
NAME_CORS=$(mpstat -P ALL | fgrep ":" | grep -v "%" | awk '{ printf "CORE[%s] ", $2}')
...
while sleep $1
 do
 	...
 	then
		...
		CPU_USAGE=$(mpstat -P ALL | fgrep ":" | grep -v "%" | awk 'function GSUB(F) {
												gsub(",",".",$F)
											    }
		...
		DATA_PLOT=$(mpstat -P ALL | fgrep ":" | grep -v "%" | awk 'function GSUB(F) {
												gsub(",",".",$F)
											    }
